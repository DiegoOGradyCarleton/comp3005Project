...
def Q_1(conn, execution_time):
    ...
    query = """SELECT players.player_name, AVG(typed_events.xg_score)
FROM events
	INNER JOIN typed_events ON typed_events.event_id = events.event_id
	INNER JOIN players ON players.player_id = events.player
WHERE events.event_type = 16
AND events.match_id IN (
	SELECT match_id FROM matches
	WHERE season_id = 90
	AND competition_id = 11)
GROUP BY players.player_name
ORDER BY AVG(typed_events.xg_score) DESC"""
...
def Q_2(conn, execution_time):
    ...
    query = """SELECT players.player_name, count(*) AS event_type 
FROM players
INNER JOIN events
ON players.player_id = events.player
WHERE events.event_type = 16
AND events.match_id IN (
	SELECT match_id FROM matches
	WHERE season_id = 90
...
AND events.match_id IN (
	SELECT match_id FROM matches
	WHERE (season_id = 90 OR season_id = 42 OR season_id = 4)
	AND competition_id = 11
	)
GROUP BY players.player_name
ORDER BY count(*) DESC"""
...
def Q_4(conn, execution_time):
    ...
    query = """SELECT teams.team_name, count(*) AS event_type 
FROM events
	INNER JOIN teams ON teams.team_id = events.team
WHERE events.event_type = 30
AND events.match_id IN (
	SELECT match_id FROM matches
	WHERE season_id = 90
	AND competition_id = 11)
...
def Q_5(conn, execution_time):
    ...
    query = """SELECT players.player_name, count(*) AS event_type 
FROM typed_events
	INNER JOIN players ON players.player_id = typed_events.id1
	NATURAL JOIN events
WHERE typed_events.event_type = 30
AND events.match_id IN (
	SELECT match_id FROM matches
	WHERE season_id = 44
...
FROM events
	INNER JOIN players ON players.player_id = events.player
	INNER JOIN typed_events ON typed_events.event_id = events.event_id
WHERE typed_events.event_type = 14
AND typed_events.id1 = 8
AND events.match_id IN (
	SELECT match_id FROM matches
	WHERE (season_id = 90 OR season_id = 42 OR season_id = 4)
	AND competition_id = 11
...
FROM events
	INNER JOIN players ON players.player_id = events.player
	INNER JOIN typed_events ON typed_events.event_id = events.event_id
WHERE typed_events.event_type = 39
AND events.match_id IN (
	SELECT match_id FROM matches
	WHERE season_id = 90
	AND competition_id = 11)
